...
@eqx.filter_jit
def val_step(model, xb, yb, pad_id: int = 0):
...
def compute_val_loss(model, x, y, pad_id: int = 0, batch_size: int = 128) -> float:
    ...
    for i in range(n_batches):
        ...
        loss = val_step(model, xb, yb, pad_id)
        total_loss += loss
...
def top_k_accuracy(model, x, y, pad_id: int = 0, k: int = 5, batch_size: int = 128) -> float:
    ...
    for i in range(n_batches):
        ...
        correct, tokens = _top_k_batch(model, xb, yb, k, pad_id)
        total_correct += correct
        total_tokens += tokens

    return float(total_correct / jnp.maximum(total_tokens, 1))